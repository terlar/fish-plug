...
VERSION := $(shell grep -Po "(?<=set -l ver ')[^']+(?=')" functions/plug.fish)
...
install:
	...
	install -m 644 man/man1/plug.1 "$(DESTDIR)$(MANDIR)/man1/plug.1"
	install -m 644 $(CURDIR)/conf.d/plug.fish "$(DESTDIR)$(FISHDIR)/vendor_conf.d/"
...
.PHONY: uninstall
uninstall:
	-rm -f "$(DESTDIR)$(FISHDIR)/vendor_conf.d/plug.fish"
	...
	rm -rf "$(DESTDIR)$(MANDIR)/man1/plug.1"
